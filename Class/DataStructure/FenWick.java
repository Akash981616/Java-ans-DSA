...
public class FenWick {
    ...
    public static void main(String[] args) {
        ...
        int res[] = FenWickTree(arr, Tree);

    }

    public static int[] FenWickTree(int arr[], int Tree[]) {

        for (int i = 1; i < arr.length; i++) {
            update(i, Tree, arr[i]);
            System.out.println(get(i, Tree));
        }
        return Tree;

    }

    public static int get(int idx, int Tree[]) {
        int sum = 0;
        while (idx > 0) {
            sum += Tree[idx];
            idx -= idx & (-idx);
    ...
    public static void update(int idx, int[] Tree, int updateValue) {
